...
void removeTail(List *&L)
{
    ...
    //1 2
    // 1 2 3 4
    if (L->p_head == L->p_tail && L->p_head != nullptr) {
        delete L->p_head;
        L->p_head = L->p_tail = nullptr;
        return;
    }
    while (tmp && tmp->p_next != L->p_tail) {
        tmp = tmp->p_next;
    }
    delete L->p_tail;
    L->p_tail = tmp;
...
void removeBefore(List *&L, int val)
{
    ...
    if (L->p_head && L->p_head->p_next && L->p_head->p_next->key == val) {
        Node* tmp = L->p_head;
        L->p_head = L->p_head->p_next;
        delete tmp;
    }
    ...
    Node* p = L->p_head;
    while (p && p->p_next && p->p_next->p_next) {
...
void removePos(List *&L, int data, int pos)
{
    ...
    //Số lần dịch chuyển node;
    Node* p = L->p_head;
    for (int i = 0; i < pos - 1; i ++) {
        p = p->p_next;
        if (p->p_next == nullptr) return;
    }

    if (p &&  p->p_next &&p->p_next->key == data) {
        Node* tmp = p->p_next;
        p->p_next = p->p_next->p_next;
        delete tmp;
    }
